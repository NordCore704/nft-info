...
const TraverseBlocks = () => {
  return (
    <section className='flex flex-col gap-5 items-center justify-center overflow-hidden p-5'>
        ...
        <Image alt='nftavatar' src={avatarYellow} className='rounded-md w-[30%] hover:w-[35%] transition hover:cursor-pointer'/>
        </div>
        <div className="flex flex-col gap-3 justify-center items-center">
            ...
            <p className='text-center w-auto lg:w-[60%]'>
                Does desired your blockchain seem rare or not so famous? We can't really fix that but we can get you info about NFTs that exist on that blockchain yes, so long as it actually exists, we got you bro🥷.
            </p>
            <Link href={'/search'} className='bg-scheme-red p-3 text-center w-64 text-lg font-bold rounded-full hover:bg-green-500 duration-300 transition-colors'>See How</Link>
        </div>
    </section>
  )
}
