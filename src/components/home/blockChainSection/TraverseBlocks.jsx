...
const TraverseBlocks = ({ montserrat, ubuntu}) => {
  return (
    <section className={`flex flex-col gap-5 items-center justify-center overflow-hidden p-5 ${montserrat.className}`}>
        ...
        <div className="flex flex-col gap-3 justify-center items-center">
            <p className={`capitalize text-3xl font-semibold text-center ${ubuntu.className}`}>Traverse Across different Blockchains</p>
            ...
            <Link href={'/search'} className={`hover:bg-scheme-green hover:text-white p-3 text-center w-64 text-lg font-bold rounded-full bg-green-500 duration-300 transition-colors ${ubuntu.className}`}>See How</Link>
        </div>
    </section>
  )
}
